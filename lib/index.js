...
async function runBeforeChecks(
    Model,
    queryContext,
    { unique: uniqueConstraints, before: beforeChecks, precedence },
    isPatch,
    oldInstance
) {
    ...
    const runner = async () => {
        ...
        const beforeFns = (beforeChecks)
            ? beforeChecks.map(fn => () => fn(this, oldInstance, queryContext))
            : [];
...
module.exports = (opts = {}) => {
    ...
    if (error) {
        throw new Error(`${error.message} on 'objection-before-and-unique' options.`);
    }
        ...
        return class extends Model {
            $beforeInsert(queryContext) {
                return Promise.resolve(
                    ...
                    return runBeforeChecks.call(this, Model, queryContext, opts);
                });
            }

            $beforeUpdate(updateOpts, queryContext) {
                if ((opts.unique || opts.before) && !updateOpts.old) {
                ...
                return Promise.resolve(
                    ...
                    return runBeforeChecks.call(this, Model, queryContext,
                        opts, updateOpts.patch, updateOpts.old);
                });
            }
        };
    };
