...
module.exports = (opts = {}) => {
  ...
  const ans = Joi.validate(opts, schema, {
    abortEarly: true,
    allowUnknown: false
  });
  ...
  if (opts.schema && !opts.schema.isJoi) {
    throw Error(
      `'schema' must be a Joi object on ${config.moduleName} options.`
    );
  }

  // Check precedence
  if (opts.precedence.first && opts.precedence.first === opts.precedence.last) {
    throw Error(
      `precedence.first can't be equal to precedence.last on ${
        config.moduleName
      } options.`
    );
  }
  ...
  if (!opts.old && opts.unique) {
    for (let obj of opts.unique) {
      if (obj.hasOwnProperty('for')) {
        throw new Error(
          `unique.for cannot be used when 'old' is set to false for ${
            config.moduleName
          }`
        );
      }
  ...
  return (Model) => {
    return class extends Model {
      $beforeInsert(queryContext) {
        return Promise.resolve(super.$beforeInsert(queryContext)).then(
          (res) => {
            ...
            return runBeforeChecks
              .call(this, Model, queryContext, opts, operation)
              .then(() => res);
          }
        );
      }
      ...
      $beforeUpdate(updateOpts, queryContext) {
        if (opts.old && (opts.unique || opts.before) && !updateOpts.old) {
          throw new Error(
        ...
        return Promise.resolve(
          ...
          const operation = updateOpts.patch ? 'patch' : 'update';
          return runBeforeChecks
            .call(this, Model, queryContext, opts, operation, updateOpts.old)
            .then(() => res);
        });
      }
    };
